...
public class TrainingHelperApp extends Application {
    ...
    private Parent buildBottomPane() {
        ...
        var pane = new Pane(lblProgressText);
    ...
    private Parent buildTopPane() {
        ...
        var hBox = new HBox(10, btnRun, btnSave, progress);
    ...
    private void prepareForTraining(String modelId, int[] inputShape, int epochs, int batchSize, File trainingDir, File testingDir) {
        ...
        try {
            var trainingIterator = DL4JHelper.createIterator(trainingDir, inputShape[1], inputShape[2], inputShape[0], batchSize, SEED);
            var testingIterator = DL4JHelper.createIterator(testingDir, inputShape[1], inputShape[2], inputShape[0], batchSize, SEED);
    ...
    private void saveSelectedDir(MouseEvent e) {
        var targetLink = (Hyperlink) e.getSource();
